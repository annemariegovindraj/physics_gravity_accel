...
Font big = new Font("Verdana", Font.PLAIN, 20); Font textfont= new Font("Verdana", Font.PLAIN, 18); FontMetrics fm; 
...
text[3]=" We have : . restoring force F=- kx . Newton's law F = ma with a=$d²x!/dt²! . . becomes : m$d²x!/dt²! +kx = 0 . We can guess that A cos(\u03C9 t) (or Bsin(\u03C9t+\u03C6) is a solution for x since . $d²x!/dt²! is -A\u03C9²cos(\u03C9t) .  . For a given mass (m) and springconstant (k), we can calculate \u03C9² : . -mA\u03C9² cos(\u03C9 t)+kAcos(\u03C9t) =0 or \u03C9²=$k!/m!";
...
text[12]="In case of a spring it is the work done to compress the spring over a distance x. The force to overcome increases with x as F= kx.  . Work done = W =\u222Bkx.dx = $kx²!/2! = $m\u03C9²x²!/2! . Potential energy is highest where displacement is highest and speed is zero. . Sum of Kinetic and Potential Energy remains constant: . $m\u03C9² (A²-x²)!/2! + $m\u03C9²x²!/2!= $m\u03C9²A²!/2! = $mkA²!/2m! = $kA²!/2!. . Next we calculate and show on graphics the displacement, speed, Kinetic Energy (Ek) and Potential Energy (Ep) for a frictionless spring and a pendulum. Note that Ek and Ep follow a simple harmonic motion with doubled frequency (remember 2sin²\u03C9t = 1-cos2\u03C9t) . You can stop the animation anytime.";
...
pendulumimg=createImage(350,620); Graphics pendg=pendulumimg.getGraphics();
 ...
 float w= (float)(Math.sqrt(g/length)); thizfont=pendg.getFont(); pendg.setFont(new Font (thizfont.getName(),Font.BOLD, 20));
...
step=10;	}
 if(step==10){
 msgimg2=textGetLines(text[12], 500);
...
//if(superimpose==true){addGreenPoint(graphim[8],7,Ek);}addGreenPoint(graphim[7], 6, dx);
 ...
 msg2.setColor(lightyellow); msg2.fillRect(0,0,195,300);
   msg2.setColor(Color.black);
  msg2.drawString("time(sec): "+(float)tt/20, 5,30); int hh=(int)((length-dy)*10000); msg2.drawString("height(cm): "+(hh/100), 5,60);int ep=(int)(Ep*100000); msg2.drawString("Ep (mJ):  "+(float)ep/100, 5,150);	
  ...
  {   jx+=lineheight;///among the [] of lines ,look for $ or % ,super or sub script. Then note string upto ! into sublines[]
       int wline=(mylines[j]).length();   int index=0, startindex=0; int Xstart=0;
		 ...
		 int Xsublines[]= new int [10];String[] sublines=new String[10]; ///place and content of non sub or super-script
               int Xfrac[]= new int[10]; String strfrac[]=new String[10]; int nbfrac=0;int strfraclength=0;
	 Xsublines[0]=8;    
...
{  if( mylines[j].charAt(index)=='$')  
 ...
 if((nbsup>0)||(nbsub>0))
  { for( int i=0; i<=(nbsub+nbsup);i++){msg.drawString(sublines[i],Xsublines[i],jx) ;}
	...
	if(nbfrac>0){for( int i=0; i< nbfrac;i++){ msg.drawLine(Xfrac[i],jx-3,Xfrac[i]+strfraclength   ,jx-3);
...
{   Graphics pg= im.getGraphics();   pg.setColor(Color.white); pg.fillRect(0,0,dwidth[jj],dheight[jj]);
 ...
 public void setAxis(Image im, float divy, double uny)
  {  Graphics pg= im.getGraphics(); pg.setFont(new Font("Dialog", Font.BOLD, 16));  // x=tt
 ...
 pg.setFont(new Font("Dialog", Font.BOLD, 16)); 
...
if (exerciseover==false)
	...
	offS.setColor(Color.yellow); offS.drawString("NEXT  ",600,height-20); offS.drawString("SAME ANIMATION  ",530,height-80); }
	break;
case 9 : case 10: 	offS.drawImage(msgimg1,(int)(width/4),10,this); offS.drawImage(msgimg2,width/2,80,this); 
       offS.drawImage(scene, 50,100,this) ;
    offS.setColor(bluegreen); offS.fillRect(390, height-50,120,60); 
	offS.setColor(Color.yellow); offS.drawString("NEXT  ",400,height-20); 
	 break; 
